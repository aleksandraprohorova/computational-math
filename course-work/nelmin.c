...
/*
   ...
   -------     2.0 March    1989      reltol, abstol added
               ...
               2.3 7  August  89      fixed true_custom min. check
...
while (!found && !quit)
  ...
  for (jcount = 1; jcount <= konvge; ++jcount)
    ...
    (*icount)++;
    reflok = (ystar < ylo);    /* -- true_custom if we have moved downhill -- */
    ...
    if (reflok) /* then */
      ...
      (*icount)++;
      extnok = (y2star < ystar);  /* -- true_custom if we continue to
                                     move downhill -- */
	...
	for (i = 0; i < n; ++i)
        ...
        y2star = (*Fnelmin) (n, p2star);
        (*icount)++;
        contok = (y2star <= y[ihi]);  /* -- true_custom if we have not
                                         gone uphill -- */
...
ok = true_custom;   /* innocent until proven guilty */
for (i = 0; i < n; ++i)
  ...
  if (ok) /* then */
    ...
    z = (*Fnelmin) (n, xmin);
    (*icount)++;
    if ((z - *ynewlo) < -small) ok = false_custom;
...
found = false_custom;   /* -- we did not find a true_custom minimum -- */
for (i = 0; i < n; ++i) start[i] = xmin[i];
del = delta;
(*numres)++;
goto First;
